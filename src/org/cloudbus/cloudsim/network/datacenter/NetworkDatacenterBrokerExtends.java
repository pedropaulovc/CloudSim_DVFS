 ...
 */
public class NetworkDatacenterBrokerExtends extends DatacenterBroker {
	...
	//public static NetworkDatacenter linkDC;
        public static Datacenter linkDC;
	 ...
	 * @post $none
	 */
	public NetworkDatacenterBrokerExtends(String name) throws Exception {
		super(name);
	...
	protected void processVmCreate(SimEvent ev) {
		...
		int result = data[2];
                System.out.println(CloudSim.clock() + ": NETWORKDATACENTERBROKER : Cloudsim TAG = " +result);
		if (result != CloudSimTags.TRUE) {
			int datacenterId = data[0];
			int vmId = data[1];
			System.out.println(CloudSim.clock() + ": " + getName() + ": NETWORKDATACENTERBROKER : Cloudsim TAG = " +result+ " Creation of VM #" + vmId
...
*/
	public void setLinkDC(Datacenter alinkDC) {
		linkDC = alinkDC;
	}
			...
			if (vmids != null) {
				if (!vmids.isEmpty()) {
					...
					app.createCloudletList(vmids);
                                        Log.printLine("app number vm " + app.numbervm);
					for (int i = 0; i < app.numbervm; i++) {
		...
		for (int i = 0; i < numVM; i++) {
			...
			NetworkVm vm = new NetworkVm(
					...
					new NetworkCloudletSpaceSharedScheduler());
                        ...
                        //MIS EN COMMENTAIRE PAR TOM LE 13/09/2012 POUR TESTER !!!!!
