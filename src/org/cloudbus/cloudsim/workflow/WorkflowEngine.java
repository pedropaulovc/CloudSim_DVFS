...
public class WorkflowEngine extends SimEntity{
	...
	public WorkflowEngine(String dagFile, long execTime, long baseMIPS, Policy policy, VMOffers vmOffers) {
	...
	protected void doProvisioning() {
		...
		//runs the policy
		policy.processDagFile(dagFile,this.getId(),execTime, baseMIPS, vmOffers);
		...
		for (Cloudlet cloudlet: getCloudletReceivedList()) {
			...
			if (cloudlet.getCloudletStatus() == Cloudlet.SUCCESS){
              ...
              /*  try {
                    ...
                    fw.write(cloudlet.getCloudletId() + indent + dft.format(cloudlet.getSubmissionTime()) + indent + dft.format(executionTime) + indent + indent + dft.format(cloudlet.getFinishTime()));
	...
	private void removeOngoingTransmission(int sourceId, int destinationId, int dataId) {
     ...
     public double computeEnergy()
     {
         ...
         String name_xml = Properties.DAG_FILE.getProperty();
         Boolean optimized = Boolean.parseBoolean(Properties.OPTIMIZE.getProperty());
         if(optimized)
         {
             String modedvfs = Properties.MODEDVFS.getProperty();
             name_xml+="_"+modedvfs;
         }
         else
             name_xml+="_Fmax";
        ...
        try {
            ...
            ex.printStackTrace();
              ...
              for(Task t : policy.tasks )
              {
               String s   = "";
              
                  s+=t.getId()+"\t";
                  s+=dft.format(t.getCloudlet().getExecStartTime())+"\t";
                  s+=dft.format(t.getCloudlet().getFinishTime())+"\t";
                  double length = t.getCloudlet().getFinishTime() - t.getCloudlet().getExecStartTime();
                  s+=dft.format(length)+"\t";
                  tmpEner=ener.getTaskEnergyConsumed(t,endTime, policy.optimizeScheduling, policy.modedvfs);
                  sumEner+=tmpEner;
                  s+=dft.format(tmpEner)+"\t";
                  s+=t.getVmId()+"\t";
                  List<Task> Parents  = t.getParents();
                  
                  for(int p = 0 ; p<Parents.size() ; p++)
                      s+=Parents.get(p).getId() +",";
